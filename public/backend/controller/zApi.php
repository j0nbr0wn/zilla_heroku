...
class zApi{
	...
	// billingPreview call wrapper
	function zBillingPreview($zBillingPreviewReq) {
		$billingPreviewWrapper = array("requests"=>$zBillingPreviewReq);
		$billingPreviewWrapper = array("BillingPreview"=>$billingPreviewWrapper);

		$billingPreviewResult;
		try{
			$billingPreviewResult = $this->client->__soapCall("BillingPreview", $billingPreviewWrapper, null, $this->header);
